...
public class WhirlisprigAbility<W extends Whirlisprig> extends IdentityAbility<W> {
    ...
    ParticleColor color = new ParticleColor(50, 250, 55);
    Spell spell = new Spell(EffectGrow.INSTANCE, AugmentAOE.INSTANCE, AugmentAOE.INSTANCE, AugmentAOE.INSTANCE);

    @Override
    public void onUse(Player player, W w, Level level) {
        EntitySpellResolver resolver = new EntitySpellResolver(new SpellContext(spell, player).withColors(color.toWrapper()));
    ...
    @Override
    public Item getIcon() {
        return ItemsRegistry.WHIRLISPRIG_CHARM;
    }
}
